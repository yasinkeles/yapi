...
class AnalyticsController {
  async getOverview(req, res, next) {
    try {
      ...
      return successResponse(res, { ...analytics, counts });
    } catch (error) {
      next(error);
    }
  }
  ...
  async getLogs(req, res, next) {
    try {
      ...
      logs.forEach(log => {
        if (log.request_params) log.request_params = JSON.parse(log.request_params);
        if (log.request_body) log.request_body = JSON.parse(log.request_body);
      });
