...
const Roles = () => {
    ...
    const toggleAllPermissions = () => {
        if (formData.permissions.length === AVAILABLE_PAGES.length) {
            setFormData(prev => ({ ...prev, permissions: [] }));
        } else {
            setFormData(prev => ({ ...prev, permissions: AVAILABLE_PAGES.map(p => p.path) }));
        }
    };
                                            ...
                                            role.permissions.map(perm => {
                                                ...
                                                return page ? (
                                                    <span key={perm} className="px-2 py-1 bg-dark-800 text-slate-300 border border-dark-700 rounded text-xs">
                                                        <i className={`bi ${page.icon} me-1`}></i>
                                                        {page.name}
                                                    </span>
                                                ) : null;
                                            })
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    >
                                        {formData.permissions.length === AVAILABLE_PAGES.length ? 'Deselect All' : 'Select All'}
                                    </button>
                                </div>
